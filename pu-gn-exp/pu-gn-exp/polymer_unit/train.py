...
try:
    from torch.utils.data._utils import collate as torch_collate_utils
    if not hasattr(torch_collate_utils, '_use_shared_memory'):
        torch_collate_utils._use_shared_memory = False
except Exception:
    pass
...
graphs_df = {k: [] for k in ['Loss', 'Nodes', 'Edges', 'Pred', 'Real']}
...
for epoch_idx in epoch_bar:
    ...
    for graphs, targets in dataloader_train:
        ...
        targets = targets.to(experiment.session.device, dtype=torch.long)
        results = model(graphs)
        logits = results.global_features
        if logits.dim() == 1:
        ...
        if experiment.session.losses.solubility > 0:
            loss_sol = criterion_mean(logits, targets)
            loss_total += experiment.session.losses.solubility * loss_sol
            loss_sol_avg.add(loss_sol.mean().item(), len(graphs))
            train_bar_postfix['target'] = f'{loss_sol.item():.5f}'
             
            if 'every batch' in experiment.session.log.when:
                logger.add_scalar('loss/train/target', loss_sol.mean().item(), global_step=experiment.samples)
    ...
    if 'every epoch' in experiment.session.log.when and 'every batch' not in experiment.session.log.when:
        ...
        if experiment.session.losses.solubility > 0:
            logger.add_scalar('loss/train/target', loss_sol_avg.get(), global_step=experiment.samples)
        if experiment.session.losses.l1 > 0:
    ...
    for batch_idx, (graphs, targets) in enumerate(dataloader_val):
        ...
        if experiment.session.losses.solubility > 0:
            ...
            val_bar_postfix['target'] = f'{loss_sol.item():.5f}'
    ...
    if len(acc_list) > 1:
        print("acc_list")
        print(acc_list)
        accuracy = torch.mean(torch.tensor(acc_list))
        print("accuracy")
        print(accuracy)
    if logger is not None and (
        ...
        if experiment.session.losses.solubility > 0:
            logger.add_scalar('loss/val/target', loss_sol_avg.get(), global_step=experiment.samples)
        if experiment.session.losses.l1 > 0:
...
experiment.loss_sol = graphs_df.Loss.mean()
print('Target MSE:', experiment.loss_sol)
