...
for path in args.data:
    ...
    if path.is_dir():
        data.extend(list(path.glob('*.pt')) + list(path.glob('*.csv')))
    elif path.is_file() and path.suffix in {'.pt', '.csv'}:
        data.append(path)
    else:
...
if data[0].suffix == '.csv':
    dataset_predict = SolubilityDataset(data[0])
else:
    dataset_predict = torch.load(data[0], map_location='cpu')
...
with tqdm.tqdm(desc='Predict', total=len(dataloader_predict.dataset), unit='g') as bar:
    for graphs, *_ in dataloader_predict:
        ...
        results = net(graphs)
        if results.node_features is not None:
            results.node_features.sigmoid_()
        elif results.global_features is not None:
            results.global_features = results.global_features.softmax(dim=-1)
