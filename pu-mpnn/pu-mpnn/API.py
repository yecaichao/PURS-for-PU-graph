...
"""
def get_node_feature0(filename):
    ...
    selector.fit(df2)
    data3 = selector.transform(df2)
    index = selector.get_support(True)
    f = np.array(df2.columns)
    select_feature = f[index.tolist()]
    df3 = df2.reindex(columns=select_feature)
    dist = {}
    for i in range(df3.shape[0]):
        dist[i]=df3.loc[i].tolist()
    return dist,df3.shape[1]
...
def bratch_select(index_data0,index_data,str_df,index_cp,bratch_cp,smiles):
    ...
    for i in cp_to_delete:
        print("Removing invalid branch entry:", i, index_data[i])
...
def get_pair_atom(total_neighbor_data,total_inner_dist,smi_list):
    pair_atom_dist = {}
    count = 0 
    for key,value in total_neighbor_data.items():
        #可视化分子结构
        smiles = smi_list[count]
        ...
        for k,v in polymer.items():
            ...
            inner_num.sort()        
            min_atom = inner_num[0]-1
            max_atom = inner_num[-1]+1
            min_nei_atom = [i.GetIdx() for i in mol.GetAtomWithIdx(inner_num[0]).GetNeighbors()]
            max_nei_atom = [i.GetIdx() for i in mol.GetAtomWithIdx(inner_num[-1]).GetNeighbors()]
            r_nei = polymer_unit['right_neighbor']
            l_nei = polymer_unit['left_neighbor']
            if r_nei == 0:
                if l_nei == 0:
                    print('Connectivity issue in polymer {} at unit {}'.format(key,k))
            if r_nei != 0:
                for i in r_nei.keys():
                    inner_num_r = inner_dist[i]
                    link_atom_list = list(set(inner_num_r) & set(min_nei_atom))+list(set(inner_num_r) & set(max_nei_atom))
                    min_atom_link = list(set(inner_num_r) & set(min_nei_atom))
                    max_atom_link = list(set(inner_num_r) & set(max_nei_atom))
                    if min_atom in inner_num_r:
                        pair_atom = [inner_num[0],min_atom]
                        pair_index = [k,i]
                        atom_dist['pair_atom'].append(pair_atom)
                        atom_dist['pair_index'].append(pair_index)
                    elif max_atom in inner_num_r:
                        pair_atom = [max_atom,inner_num[-1]]
                        pair_index = [k,i]
                        atom_dist['pair_atom'].append(pair_atom)
                        atom_dist['pair_index'].append(pair_index)
                    elif len(link_atom_list)== 1:
                        if len(max_atom_link) == 1:
                            pair_atom = [max_atom_link[0],max_atom]
                            pair_index = [k,i]
                            atom_dist['pair_atom'].append(pair_atom)
                            atom_dist['pair_index'].append(pair_index)
                        elif len(min_atom_link) == 1:
                            pair_atom = [min_atom,min_atom_link[0]]
                            pair_index = [k,i]
                            atom_dist['pair_atom'].append(pair_atom)
                            atom_dist['pair_index'].append(pair_index)
        ...
        #print("$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$")
        #minus = len(polymer) - num_pair
        #if minus > 1:
        #    print('{}单体,一共{}个基元,找到{}个原子对'.format(key,len(polymer),num_pair))
        #    print("pair_atom")
        #    print(atom_dist['pair_atom'])
        #    for atom in mol.GetAtoms():
        #        atom.SetProp("atomNote", str(atom.GetIdx()))
        #    fig = Draw.MolToImage(mol, size=(2000,2000), kekulize=True)
        #    #display(fig)
    return pair_atom_dist



def get_adj(ring_total_list,total_neighbor_data,name_list,same_shape=False):
    ###生成连接矩阵(矩阵维度根据原子数)
    ...
    for idx,i in enumerate(name_list):
        ...
        elif same_shape==True:
            n_max = max([len(i) for i in total_neighbor_data.values()])+1
...
def get_pu(smi_list,name_list):
    ...
    #找到所有括号的索引
       ...
       #创建一个所有非最小括号的括号字典
        ...
        neighbor_data2=found_end_point_neighbour(smiles,neighbor_data,index_data3) 
    #去除'[C]'
...
def get_sdf(mol_list,name_list,sdf_name):
    #将三维化的mol文件输出
    #sdf_name e.g.'DATA/polymer.sdf'
...
def gen_wl_in(ring_total_list):
    
    #处理数据
    smi_list=[]
    mol_list = []
    adj_list = []
    for i in ring_total_list:
        #Rdkit识别不出顺反，要把'/'与'\'去掉
        #if '/'in i[1]:
            #i[1]=i[1].replace('/','')
        #if '\\'in i[1]:
    ...
    #将邻接矩阵转换为边列表
    c = torch.tensor(c,dtype=int)
    nozero=torch.nonzero(c)
    it=c.T
    nozreo2=torch.nonzero(it)
    edge=torch.cat([nozero,nozreo2],dim=0)
    edge=(torch.unique(edge,dim=0)).T
    
    #生成邻接矩阵
    with open('output/polymer_A.txt','w') as f:
        for i in edge.T:
            f.write(str(int(i[0])))
            f.write(',')
            f.write(str(int(i[1])))
            f.write('\n')    
    #生成节点索引
    with open('output/polymer_graph_indicator.txt','w') as f:
    ...
    for idx,mol in enumerate(mol_list):
        ...
        for n in range(n_atom):
            ...
            atom = a.GetSymbol()
            atom_idx = atom_list.index(str(atom))
            node_list.append(atom_idx)
            
    with open('output/polymer_node_labels.txt','w') as f:
        for i in node_list:
            f.write(str(i))
            f.write('\n')
            
    #生成边标签
    bond_type_list = ['AROMATIC','SINGLE', 'DOUBLE', 'TRIPLE']
    bond_index_list = []
    for adx,adj in enumerate(adj_list):
        mol = mol_list[adx]
        for idx,i in enumerate(adj):
            index = np.nonzero(i)
            for j in index[0]:
                bondpath = Chem.GetShortestPath(mol,idx,int(j))
                pair = [idx,int(j)]
                bonds = [mol.GetBondBetweenAtoms(bondpath[t], bondpath[t + 1]) for t in range(len(bondpath) - 1)]
                if len(bonds)==1:
                    bond_type = str(bonds[0].GetBondType())
                    bond_index = bond_type_list.index(bond_type)
                    bond_index_list.append(bond_index)
                       
    with open('output/polymer_edge_labels.txt','w') as f:
...
def block_diagonal_matrix(a,b):
    #将邻接矩阵合并为分块对角矩阵
    A = a.shape[0]
    B = b.shape[0]
    zero1 = np.zeros([B,A])
    zero2 = np.zeros([A,B])
    a2 = np.hstack((a,zero2))
    b2 = np.hstack((zero1,b))
    c = np.vstack((a2,b2))
    return c
...
def get_mpnn_input(csv_name,sdf_name):
    ...
    smi_list,name_list0,name_list,mol_list,num_list=process_smiles(csv_name)
    ...
    pair_atom_dict = get_pair_atom(total_neighbor_data,total_inner_dist,smi_list)
    matrix_list=get_adj(ring_total_list,total_neighbor_data,name_list,same_shape=True)
    MACCS = get_MACCS(ring_total_list,False)
    pu_feature = pd.DataFrame(MACCS).T
    get_sdf(mol_list,name_list,sdf_name)
    structure = Chem.SDMolSupplier(sdf_name,removeHs=False)
    n_max = max([len(i) for i in total_neighbor_data.values()])+1
    dim_node = pu_feature.shape[1]
    
    return total_inner_dist,pu_dict,pair_atom_dict,matrix_list,pu_feature,structure,n_max,dim_node
...
def main(file_name):
    ...
    matrix_list=get_adj(ring_total_list,total_neighbor_data,name_list)
    print("Assigning polymer-unit types to nodes...")
    pu_index = get_pu_dict(total_neighbor_data,ring_total_list)
    print("Finding linked atom indices between nodes...")
    pair_atom_dist = get_pair_atom(total_neighbor_data,total_inner_dist)
    print("Generating MACCS fingerprints for polymer units (47 selected bits)...")
    MACCS_dict = get_MACCS(ring_total_list,47)
    ...
    node_index, node_num_list = get_node_index(total_neighbor_data,ring_total_list)
    print("Generating edge features...")
    edge_list = get_edge_feature(mol_list,name_list,matrix_list,pu_index,pair_atom_dist,total_inner_dist)
